...
describe('productsSlice', () => {
  ...
  it('products/create/pending activa isActioning', () => {
    const store = makeStore();
    store.dispatch({ type: 'products/create/pending' });
    expect(store.getState().slice.isActioning).toBe(true);
  });

  it('products/create/fulfilled desactiva isActioning', () => {
    const store = makeStore();
    store.dispatch({ type: 'products/create/pending' });
    store.dispatch({ type: 'products/create/fulfilled', payload: {} });
    expect(store.getState().slice.isActioning).toBe(false);
  });

  it('products/create/rejected desactiva isActioning', () => {
    const store = makeStore();
    store.dispatch({ type: 'products/create/pending' });
    store.dispatch({ type: 'products/create/rejected', payload: 'error' });
    expect(store.getState().slice.isActioning).toBe(false);
  });

  it('products/update/pending activa isActioning', () => {
    const store = makeStore();
    store.dispatch({ type: 'products/update/pending' });
    expect(store.getState().slice.isActioning).toBe(true);
  });
