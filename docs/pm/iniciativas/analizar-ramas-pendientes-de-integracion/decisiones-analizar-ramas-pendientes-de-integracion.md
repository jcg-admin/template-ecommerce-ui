...
| Razon | Los flujos de runtime estan cubiertos por los casos de uso (UC-*) del repo backend `ecommerce-doc`. Duplicar aqui produce ruido y dos fuentes de verdad. Los pocos flujos que son **exclusivamente del UI** (manejo de 401, mock-first) se documentan en `conceptos-transversales/` con sequenceDiagram. |
