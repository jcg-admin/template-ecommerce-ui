...
| UC-CFG-05 | Gestionar datos de contacto | IMPLEMENTADO | `AdminSystemSettingsPage` edita `support_email`/`phone`/`address` + `social_links{}`; `Footer` y `ContactPage` los muestran vía `usePublicSettings` (`GET /settings/public/`, POST-02). Cerrado en `cerrar-gaps-ucs-auditoria` F4. |
