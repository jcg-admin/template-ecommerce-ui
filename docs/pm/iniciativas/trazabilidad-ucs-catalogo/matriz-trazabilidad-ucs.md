...
> **UC-CFG-05** (campos de redes perdidos en DR-02). El resto de discrepancias
...
| UC-AUTH-16 | dar de baja cuenta | IMPLEMENTADO | `SecurityPage` con campo de contraseña → `deleteAccount({password})` → `POST /api/v1/auth/me/deactivate/` (mock valida password, 400 si incorrecto). Cerrado en `cerrar-gaps-ucs-auditoria` F2. |
...
| UC-ORD-04 | Cancelar orden (cliente) | IMPLEMENTADO | `cancelOrder` (`ordersSlice.js`) ahora cableado en `OrderDetailPage` (SupportCard): botón "Cancelar pedido" gateado por estado PENDING/PROCESSING → ConfirmModal → `POST /orders/<n>/cancel/`. Cerrado en `cerrar-gaps-ucs-auditoria` F1. |
...
| UC-CFG-05 | Gestionar datos de contacto | IMPLEMENTADO | `AdminSystemSettingsPage` edita `support_email`/`phone`/`address` + `social_links{}`; `Footer` y `ContactPage` los muestran vía `usePublicSettings`. Endpoint canónico `GET/PATCH /api/v1/config/settings/` (rest-api-conventions.rst:92). Cerrado en `cerrar-gaps-ucs-auditoria` F4 + corrección de refs. |
