...
| [adaptar-componentes-kno-react](adaptar-componentes-kno-react/) | En ejecucion | Solicitud usuario | Adaptar de manera NATIVA al template componentes con encaje ecommerce inspirados en el repo de referencia NestorMonroy/-progress (catalogo modelado sobre KendoReact): codigo propio, sin importar el origen. F1 hecha: 4 componentes nativos con tests (ProductGallery, FileUpload, GanttChart, PdfViewer). F2 en curso: ProductGallery integrado en ProductPage (UC-CAT-GAL); resto de integraciones pendientes. | |
