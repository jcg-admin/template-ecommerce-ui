...
| [unificar-navs-cuenta](unificar-navs-cuenta/) | Planeada | Drift DR-01 (cablear-rutas-huerfanas) | Doble nav de cuenta: AccountLayout (canonica, router-level, con test, 9 entradas) y AccountSidebar (legacy, embebida en 8 paginas, 6 entradas con contadores) se renderizan a la vez con listas distintas. Plan: migrar addresses+contadores a AccountLayout, quitar AccountSidebar de las 8 paginas, borrar el componente. F1-F4 con TDD. Sin ejecutar. |
| [unificar-sitesettings](unificar-sitesettings/) | Planeada | Drift DR-02 (cablear-rutas-huerfanas) | Dos paginas admin editan el mismo singleton (GET/PATCH /api/v1/admin/settings/): AdminSystemSettingsPage (UC-ADM-04, con test, settingsSlice) y AdminSiteSettingsPage (4 tabs, sin test, adminSlice.siteSettings). Recomendacion: canonica = AdminSystemSettingsPage; reconciliar campos de las 4 tabs antes de borrar la duplicada, repuntar hub a /admin/system-settings, limpiar codigo muerto. F1-F4. Sin ejecutar. |
