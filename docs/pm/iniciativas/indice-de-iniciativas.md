...
| [auditar-y-corregir-inconsistencias](auditar-y-corregir-inconsistencias/) | En ejecucion | (raiz) | Los handlers MSW interceptan paths distintos a los que la app realmente llama. H-01 CRITICA: `catalog.ts` intercepta `/api/products/` pero la app llama a `/api/v1/catalogue/`. H-02: handlers legacy en `auth.ts`. H-04: `payments.ts` con paths incorrectos. H-05: wishlist path sin `/v1/`. H-06: `API_BASE` codigo muerto. H-07: proxy devServer fallback sin documentar. | |
